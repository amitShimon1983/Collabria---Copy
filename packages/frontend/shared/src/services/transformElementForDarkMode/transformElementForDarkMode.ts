...
export const transformElementForDarkMode = (element: HTMLElement, baseColor: string, useSimpleMethod?: boolean) => {
  ...
  try {
    ...
    const attrBGColor = element.getAttribute('bgcolor');
...
export function fixContrast(color: Color, comparisonColor: Color, useSimpleMethod: boolean, baseColor: string): Color {
  const baseBGColor = Color(baseColor);
  ...
  if (useSimpleMethod) {
    ...
    if (comparisonLValue > midpointLValue) {
      ...
      newLValue =
        ((newLValue - baseLValue) * (ceilingDarkLValue - baseLValue)) / (midpointLValue - baseLValue) + baseLValue;
    } else {
